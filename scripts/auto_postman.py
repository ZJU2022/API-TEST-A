...
def generate_test_cases(api_doc_path, output_dir=None):
    """生成测试用例"""
    cmd = [sys.executable, "-m", "src.main", "generate", "-f", api_doc_path]
    if output_dir:
        cmd.extend(["-o", output_dir])
    
    cmd_str = " ".join(cmd)
    print(f"生成测试用例: {cmd_str}")
...
def main():
    parser = argparse.ArgumentParser(description="自动化API测试流程")
    parser.add_argument("-f", "--file", required=True, help="API文档或架构文件路径")
    parser.add_argument("-o", "--output", help="输出目录")
    parser.add_argument("-u", "--url", help="测试的API基本URL")
    ...
    # 1. 生成测试用例
    test_cases_path = generate_test_cases(args.file, args.output)
